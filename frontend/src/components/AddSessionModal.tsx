...
import { type Patient, createSession } from "../api";
...
export default function AddSessionModal({ patient, onClose, onSuccess }: AddSessionModalProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [formData, setFormData] = useState({
    preWeight: patient.dryWeight, 
    postWeight: "",
    systolicBp: "",
    diastolicBp: "",
    machineId: "MACH-01",
    ...
    endTime: new Date(new Date().getTime() + 4 * 60 * 60 * 1000).toISOString().slice(0, 16), 
    status: "COMPLETED"
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Failed to create session. Please check inputs.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[95vh] border border-white/20">
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white">
          <div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Clinical Intake</h2>
            <p className="text-sm text-slate-500 font-medium mt-0.5">Recording for {patient.firstName} {patient.lastName}</p>
          </div>
          ...
          <form id="session-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input required type="text" name="machineId" value={formData.machineId} onChange={handleChange} className="w-full px-4 py-3.5 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-100 outline-none transition-all font-bold text-slate-700" placeholder="e.g. M-01" />
              </div>
            </div>
            ...
            <div className="p-6 bg-indigo-50/50 rounded-[2.5rem] space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input type="number" name="systolicBp" value={formData.systolicBp} onChange={handleChange} className="w-full px-4 py-3.5 bg-white border-transparent shadow-sm rounded-2xl focus:ring-4 focus:ring-indigo-200 outline-none transition-all font-black text-indigo-700 text-lg" placeholder="120" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-indigo-400 ml-1">Post Diastolic BP</label>
                  <input type="number" name="diastolicBp" value={formData.diastolicBp} onChange={handleChange} className="w-full px-4 py-3.5 bg-white border-transparent shadow-sm rounded-2xl focus:ring-4 focus:ring-indigo-200 outline-none transition-all font-black text-indigo-700 text-lg" placeholder="80" />
                </div>
              </div>
            </div>
        ...
        <div className="px-8 py-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-4">
          ...
          <button form="session-form" type="submit" disabled={loading} className="px-10 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-sm font-black shadow-xl shadow-indigo-100 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 flex items-center gap-2">
